...
clc;clear;
c0=299792458; 
gx=0.5550; % beam pattern along-track
gy=9.97e-5; % beam pattern cross-track
freq=930e6; % frequency, L
w0=2*pi*freq;
tp = 1.67e-9; % tp is tau_p in equation (17) of (Picardi, 2004).  pulse length = 4*tp
theta0=0; % incident angle

sig1= 1 /100; % upper surface variance
L1= 30 /100; % upper correlation length
sig2_cm = 1; % lower surface variance in cm
sig2= sig2_cm /100; % lower surface variance in meter
L2= 20 /100; % lower correlation length
h0=250000; % orbit height
...
P1(6)=theta0; P1(7)=sig1;  P1(8)=L1; P1(9)=c0;  

c2=c0/real(sqrt(ep2r));
P2=zeros(1,10); % array of input parameters
P2(1)=w0;  P2(2)=tp;  P2(3)=gx;  P2(4)=gy;  P2(5)=h0; 
P2(6)=theta0; P2(7)=sig2; P2(8)=L2;  P2(9)=c2;

tau= (0*tp): ( 0.5* tp) : (20*tp); % x-axis, time difference from 1st pulse peak
d=2; % layer depth
...
%% coefficients of Regolith Layer
function [atv,ath,a,delay,Tv12,Tv21,Th12,Th21,Rv12,Rh12,Rv23,Rh23]...
    =RL(freq,d,theta1,ep2r,ep3r,sig)
% a = attenuation rate (power loss is double wave loss)
% atv, ath = 2-way attenuation, transmissivity loss included
...
a=2*ik2; % attenuation rate (power loss is double wave loss)
ad=a*d; % 1-way attenuation
...
atv = exp( -2*(k0-rk2)^2*sig^2 )*Tv12*Tv21*exp(-ad*(1/cso + 1/cs2)); % total attenuation_v
ath = exp( -2*(k0-rk2)^2*sig^2 )*Th12*Th21*exp(-ad*(1/cso + 1/cs2)); % total attenuation_h
v2=2*pi*freq/rk2;
delay=2*d/v2;
end
...
% Is1, Is2, Is3 = 1st 2nd 3rd order backscattering intensity
function[Is0, Is1, Is2, Is3, Is_inco, Is]=NM(tau,P)
...
step1=(max1-min1)/sn0;
 ...
 for a=1:sn0
   ...
   pieceInt(a) =step1*  (part1+part2+part3) /6 ;
 end  
 I0=sum(pieceInt,"all");
